...
public class FrmHopDongTraGop extends JFrame implements ActionListener,MouseListener{
	...
	public FrmHopDongTraGop() throws HeadlessException{
		setSize(1055,700);
		setVisible(true);
		...
		pnlLeft.add(txtTim=new JTextField("Tìm kiếm			"));
		...
		frmmh =new FrmMuaHang();
		frmmh.getButton1().addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				txtMaXe.setText(frmmh.getMaXe());
				txtTenKH.setText(frmmh.getTenKH());
				txtTenXe.setText(frmmh.getTenXe());
				txtSoLuong.setText(frmmh.getSoLuong());
				txtDonGia.setText(frmmh.getDonGia());
				txtThanhTien.setText(frmmh.getThanhTien());
				txtNgayLap.setText(frmmh.getNgayLap());
				txtMaHD.setText((frmmh.getMaHopDong()));
			}
		});
		 ...
		 if(o.equals(btnXoa)) {
			...
			if(row>=0) {
			 ...
			 try {
				 ...
				 int lantra=Integer.parseInt(txtlanTra.getText());
				 Double sotientragop=Double.parseDouble(txtSoTienTra.getText());
				 String ngaylap=txtNgayLap.getText();
				 HopDongTraGop hd=new HopDongTraGop(mahd, maxe, tenkh, tenxe, soluong, dongia, thanhtien, lantra, sotientragop, ngaylap);
				 if(dshdtg.create(mahd, maxe, tenkh, tenxe, soluong, dongia, thanhtien, lantra, sotientragop, ngaylap) && dshdtg.them(hd)) {
					 Object[] rowData= {mahd, maxe, tenkh, tenxe, soluong, dongia, thanhtien, lantra, sotientragop, ngaylap};
	...
	private void updateTableData() {
		...
		for(HopDongTraGop hd : list) {
			String[] rowData= {hd.getMaHD(),hd.getMaXe(),hd.getTenKH(),hd.getTenXe(),hd.getSoLuong()+"",en.format(hd.getDonGia())+"",hd.getThanhTien()+"",hd.getLanTra()+"",hd.getSoTienTra()+"",hd.getNgayLap()};
			dataModel.addRow(rowData);
		}

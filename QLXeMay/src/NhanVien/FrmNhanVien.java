...
public class FrmNhanVien extends JFrame implements ActionListener, MouseListener{
	...
	public FrmNhanVien(){
		...
		String[] cbxStr = {"Mã nhân viên","Tên nhân viên"};
...
public void actionPerformed(ActionEvent e) {
	...
	if(o.equals(btnTimKiem))
	{
		...
		String maXe = txtTimKiem.getText();
		tableNV.clearSelection();
		
		String ktma = "";
		for (int i = 0; i < tableNV.getRowCount(); i++) {
			ktma = tableNV.getValueAt(i, 0).toString();
			if(ktma.equals(maXe))
			{
...
//(String maNV, String tenNV, String diaChi, String dienThoai, String chucVu, String phongBan, String trinhDo, String bacTho, String namKN)
				...
				{
					JOptionPane.showMessageDialog(this, "Ma NV khong duoc bo trong");
				}
	...
	private boolean validData() {
		...
		String bacTho = txtBacTho.getText().trim();
		...
		if(!(diaChi.length()>0)) {
			JOptionPane.showMessageDialog(this, "Địa chỉ nhân viên không được bỏ trống");
		...
		if(cbxChucVu.getSelectedItem()=="Nhân viên hành chính"){
			if(!(phongBan.length()>0)) {
				JOptionPane.showMessageDialog(this, "Phòng ban nhân viên không được bỏ trống");
				txtPhongBan.requestFocus();
				return false;
			}
			if(!(trinhDo.length()>0)) {
				JOptionPane.showMessageDialog(this, "Trình độ nhân viên không được bỏ trống");
				txtTrinhDo.requestFocus();
				return false;
			}
		}else if(cbxChucVu.getSelectedItem()=="Nhân viên kĩ thuật"){
			if(!(bacTho.length()>0)) {
				JOptionPane.showMessageDialog(this, "Bậc thợ nhân viên không được bỏ trống");
				txtBacTho.requestFocus();
				return false;
			}
			if(!(namKN.length()>0)) {
				JOptionPane.showMessageDialog(this, "Năm kinh nghiệm nhân viên không được bỏ trống");
				txtNamKN.requestFocus();
				return false;
			}
		}
